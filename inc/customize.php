	...
	*/
	function siggen_curtomize_register( $wp_customize ){
		...
		$wp_customize->add_control( 'breadcrumbs', array(
			'label'			=> __('Display breadcrumbs', 'siggen'),
		...
		// Center widgets
		$wp_customize->add_setting( 'center_widgets', array(
			'default'	=> true,
			'sanitize_callback' => 'siggen_sanitize_checkbox'
		) );
	
		$wp_customize->add_control( 'center_widgets', array(
			'label'			=> __('Center widgets', 'siggen'),
			'section'		=> 'design_options',
			'type'			=> 'checkbox',
		) );
